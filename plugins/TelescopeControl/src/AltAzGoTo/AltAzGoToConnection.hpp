...
#ifndef AltAzGoToCONNECTION_HPP
#define AltAzGoToCONNECTION_HPP
...
//! Serial port connection to a Meade AltAzGoTo or a compatible telescope.
class AltAzGoToConnection : public SerialPort
{
public:
	AltAzGoToConnection(Server &server, const char *serial_device);
	void sendGoto(unsigned int ra_int, int dec_int);
	void sendCommand(AltAzGoToCommand * command);
...
private:
	list<AltAzGoToCommand*> command_list;
