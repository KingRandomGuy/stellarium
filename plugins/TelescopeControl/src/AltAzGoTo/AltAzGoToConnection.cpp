...
AltAzGoToConnection::AltAzGoToConnection(Server &server, const char *serial_device) : SerialPort(server, serial_device)
{
...
void AltAzGoToConnection::resetCommunication(void)
{
	...
	#ifdef DEBUG4
	*log_file << Now() << "AltAzGoToConnection::resetCommunication" << endl;
	#endif
	...
	read_timeout_endtime = 0x7FFFFFFFFFFFFFFFLL;
	goto_commands_queued = 0;
	static_cast<TelescopeClientDirectAltAzGoTo&>(server).communicationResetReceived();
}

//! Commands the telescope to slew to the given right ascension and declination.
//! Converts the coordinates and queues the necessary sequence of AltAzGoToCommand
//! objects to perform the slew.
void AltAzGoToConnection::sendGoto(unsigned int ra_int, int dec_int)
{
	if (goto_commands_queued <= 1)
	{
		...
		if (ra >= 86400)
			ra -= 86400;
		sendCommand(new AltAzGoToCommandStopSlew(server));
		sendCommand(new AltAzGoToCommandSetSelectedRa(server, ra));
		sendCommand(new AltAzGoToCommandSetSelectedDec(server, dec));
		sendCommand(new AltAzGoToCommandGotoSelected(server));
		goto_commands_queued++;
	}
	else
	{
		#ifdef DEBUG4
		*log_file << Now() << "AltAzGoToConnection::sendGoto: ignoring command" << endl;
		#endif
	}
}

bool AltAzGoToConnection::writeFrontCommandToBuffer(void)
{
...
#ifdef DEBUG4
		/*
		*log_file << Now() << "AltAzGoToConnection::writeFrontCommandToBuffer("
	...
	if (rval)
	{
		...
		#ifdef DEBUG4
		*log_file << Now()
		          << "AltAzGoToConnection::writeFrontCommandToBuffer("
		          << (*command_list.front())
		          << "): queued"
		          << endl;
		#endif
...
void AltAzGoToConnection::dataReceived(const char *&p, const char *read_buff_end)
{
	if (isClosed())
	{
		*log_file << Now() << "AltAzGoToConnection::dataReceived: strange: fd is closed" << endl;
	}
	else if (command_list.empty())
	{
		...
		else
		{
			#ifdef DEBUG4
			*log_file << Now()
			          << "AltAzGoToConnection::dataReceived: error: command_list is empty"
			          << endl;
			#endif
			resetCommunication();
		}
	...
	{
		*log_file << Now() << "AltAzGoToConnection::dataReceived: "
		                      "strange: command("
		                   << *command_list.front()
		                   << ") needs no answer"
		                   << endl;
	...
	else
	{
		while(true)
		{
			...
			{
				*log_file << Now()
				          << "AltAzGoToConnection::dataReceived: "
...
void AltAzGoToConnection::prepareSelectFds(fd_set &read_fds,
                                       fd_set &write_fds,
                                       int &fd_max)
{
	...
	if (!command_list.empty() && GetNow() > read_timeout_endtime)
	{
		if (command_list.front()->shortAnswerReceived())
		{
			...
			#ifdef DEBUG4
			*log_file << Now() << "AltAzGoToConnection::prepareSelectFds: "
			                      "dequeueing command("
			                   << *command_list.front()
			                   << ") because of timeout"
			                   << endl;
...
void AltAzGoToConnection::flushCommandList(void)
{
	if (!command_list.empty())
	{
		while (!command_list.front()->hasBeenWrittenToBuffer())
		{
			...
			{
				//*log_file << Now()
				//          << "AltAzGoToConnection::flushCommandList: "
				//          << (*command_list.front())
			...
			{
				//*log_file << Now() << "AltAzGoToConnection::flushCommandList: "
				//                   << (*command_list.front())
...
void AltAzGoToConnection::sendCommand(AltAzGoToCommand *command)
{
	if (command)
	{
		#ifdef DEBUG4
		*log_file << Now()
		          << "AltAzGoToConnection::sendCommand("
		          << *command
		          << ")"
		          << endl;
		#endif
		...
		flushCommandList();
		//*log_file << Now() << "AltAzGoToConnection::sendCommand(" << *command << ") end"
		//          << endl;
	}
}
